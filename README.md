# UNIMOL Exam Documentation for "Innovazione digitale e intelligenza artificiale nel turismo" Course

## Index

1. [Fast Links Recap](#fast-links-recap)
1. [AI involved in this Exam](#ai-involved-in-this-exam)
1. [Logo Generation](#logo-generation-chatgpt--dall-e)
1. [Video Generation](#video-generation-chatgpt--z-image--nanobanana)

## Fast Links Recap

- Intro Presentation: [Click Here](https://claude.ai/)
- Website hosted on Vercel: [Click Here](https://claude.ai/)
- Event Flyer: [Click Here](https://chatgpt.com/)

## AI involved in this Exam

- Coding: [Claude](https://claude.ai/)
- Textual Content: [ChatGPT](https://chatgpt.com/)
- Upscaling Pictures: [Nero Software](https://ai.nero.com/image-upscaler)
- Image Generation: [Nero Software](https://ai.nero.com/image-upscaler)
- Video Generation: [Nero Software](https://ai.nero.com/image-upscaler)
- Voice Generation: [Nero Software](https://ai.nero.com/image-upscaler)
- Music Generation: [Nero Software](https://ai.nero.com/image-upscaler)

## Logo Generation (ChatGPT / DALL-e)

- Original Picture

![Imag Reale](/public/real-logo-origin.webp)

- Logo Generated with DALL-e

![Logo AI](/public/logo.png)

## Prompt used for Parallax in HomePage (Gemini)

_(Alcuni prompt sono stati lievemente modificati per creare un'esperienza più realistica)_

1. **Il Selfie del Pescatore Anziano:** Un primo piano di un anziano pescatore termolese, con la pelle segnata dal sole e un berretto sdrucito. Si sta facendo un selfie goffo (inquadratura storta) a bordo di un peschereccio strapieno di gente. Sullo sfondo, fuori fuoco, si vede la "Barca del Santo" decorata e il mare blu. L'espressione è un sorriso sdentato e felice.

   - ![AI Generated Img](/public/parallax-home/parallax-1.webp)

2. **La Coppia di Turisti sulla Barca d'Appoggio:** Un selfie grandangolare scattato con un bastone per selfie da una giovane coppia di turisti su una delle barche che seguono il corteo. Hanno occhiali da sole, capelli al vento e sono circondati da altra gente. Sullo sfondo si vede la scia della barca e il porto di Termoli che si allontana. Luce solare diretta e forte.

   - ![AI Generated Img](/public/parallax-home/parallax-2.webp)

3. **Il "Wefie" dell'Equipaggio:** Un selfie di gruppo ("wefie") scattato da un membro dell'equipaggio della barca che porta il Santo. Sono 4-5 uomini in magliette coordinate, sudati, che ridono. La statua del Santo è parzialmente visibile dietro di loro, magari coperta da un telo trasparente se c'è vento. L'inquadratura è stretta e caotica.

   - ![AI Generated Img](/public/parallax-home/parallax-3.jpg)

4. **L'Attesa al Porto (Famiglia):** Una foto scattata da una mamma che ritrae la sua famiglia (marito e due bambini piccoli con cappellini) seduti sul molo del porto, in attesa del rientro della statua. I bambini mangiano un gelato che cola. La folla è pressante intorno a loro. La luce è quella calda del tardo pomeriggio.

   - ![AI Generated Img](/public/parallax-home/parallax-4.webp)

5. **Selfie allo Specchio nel Borgo:** Tre ragazze adolescenti del posto, vestite eleganti per la serata, si fanno un selfie allo specchio nella vetrina di un negozio nelle viuzze strette del Borgo Antico. Il riflesso mostra le luminarie della festa e la gente che passa dietro di loro.

   - ![AI Generated Img](/public/parallax-home/parallax-5.webp)

6. **La Foto della Nonna alla Processione di Terra:** Una foto "rubata" (non un selfie) che mostra le mani di un'anziana signora che regge uno smartphone, cercando tremolante di filmare la statua di San Basso che passa in processione per le strade della città. Lo schermo del telefono è acceso e mostra la scena.

   - ![AI Generated Img](/public/parallax-home/parallax-6.webp)

7. **Street Food e Caos:** Un ragazzo che si fa un selfie mentre addenta un panino con la porchetta o frittura di pesce, con un'espressione godereccia. È in mezzo alla calca vicino alle bancarelle. Lo sfondo è un mix sfocato di luci al neon delle bancarelle e teste di persone. Scattata con flash diretto per un look realistico "notturno".

   - ![AI Generated Img](/public/parallax-home/parallax-7.webp)

8. **La Coppia davanti alla Cattedrale:** Una coppia di mezza età, ben vestita, si scatta un selfie posato e sorridente davanti alla facciata della Cattedrale di Termoli illuminata a festa, prima della messa solenne. L'inquadratura è dal basso verso l'alto per includere la chiesa.

   - ![AI Generated Img](/public/parallax-home/parallax-8.webp)

9. **Selfie Romantico in Spiaggia:** Una coppia giovane seduta sulla sabbia della spiaggia a sud del porto. Il selfie è in controluce: i loro volti sono illuminati dall'esplosione colorata di un grande fuoco d'artificio nel cielo sopra il mare dietro di loro.

   - ![AI Generated Img](/public/parallax-home/parallax-9.webp)

10. **Il Gruppo di Amici sul Molo:** Un selfie di gruppo rumoroso di 5-6 amici ventenni ammassati sul molo. Ridono, alcuni con una birra in mano. Il flash dello smartphone illumina i loro volti, mentre dietro il cielo è pieno di fumo e scintille dei fuochi.

- ![AI Generated Img](/public/parallax-home/parallax-10.webp)

11. **La Prospettiva dal Balcone:** Un selfie scattato da una persona sola da un balcone di un palazzo che affaccia sul mare. La persona sorride guardando la camera, mentre alle sue spalle si vede la folla minuscola sulla spiaggia e i fuochi che esplodono ad altezza occhi.

- ![AI Generated Img](/public/parallax-home/parallax-11.webp)

12. **La Preparazione della Barca (Mattina presto):** Un selfie di un volontario giovane, all'alba, mentre sta finendo di mettere le bandierine sulla barca. La luce è soffusa, mattutina, il porto è ancora tranquillo. Ha l'aria stanca ma soddisfatta.

- ![AI Generated Img](/public/parallax-home/parallax-12.webp)

13. **Turista Solitaria al Tramonto:** Una viaggiatrice zaino in spalla si fa un selfie panoramico dai muraglioni del Borgo Antico al tramonto. Il sole sta calando dietro il porto pieno di barche pronte per la festa. La luce è dorata (golden hour).

- ![AI Generated Img](/public/parallax-home/parallax-13.webp)

14. **I Bambini e i Palloncini:** Una foto realistica scattata di sera da una mamma ai suoi due figli piccoli, seduti su una panchina durante la festa. I bambini sorridono felici, circondati da un grappolo di palloncini colorati e giocattoli luminosi. Lo sfondo è pieno di luci delle bancarelle e gente che passeggia. La foto ha una luce calda e naturale.

- ![AI Generated Img](/public/parallax-home/parallax-14.webp)

15. **Il Selfie con il Trabucco:** Due amici si fanno un selfie con uno storico Trabucco di Termoli sullo sfondo, mentre le prime luci della sera e della festa iniziano ad accendersi. Unisce il simbolo della città all'atmosfera festiva.

- ![AI Generated Img](/public/parallax-home/parallax-15.webp)

## Video Generation (ChatGPT / Gemini)

All the the scenes for the video were scripted by

[ChatGPT](https://chatgpt.com/)

then I used scripts to generate prompts to generates images and animate them with

[Gemini](https://gemini.google.com/)

[Full video is available here on Vimeo](https://gemini.google.com/)

### General Advertise Spot Script

```md
Spot – Festa di San Basso (cinematografico, ≈30”)

0:00
[Musica – lenta, profonda]

0:02
A Termoli, il mare ricorda.

0:05
Ricorda le luci accese sul porto,
le voci che si cercano,
il tempo che rallenta.

0:10
Ricorda una storia che torna ogni anno,
e passa sull’acqua,
tra barche e silenzi.

0:16
È una città che si ferma,
che guarda,
che aspetta.

0:21
È la Festa di San Basso.

0:24
Non è solo una celebrazione.
È un momento che resta.
```

### Advertise Spot Timing

```md
INTRO – Atmosphere (≈6 s)

1. Sea at dawn → 2.0 s
2. Port awakening → 1.5 s
3. Water detail → 1.0 s
4. People waiting → 1.5 s

DEVELOPMENT – Tradition and community (≈12 s)

5. Bells → 1.0 s
6. Preparations at the port → 1.5 s
7. Decorations → 1.0 s
8. Statue (first appearance) → 1.5 s
9. Hand in the water → 1.0 s
10. Boat departing → 1.5 s
11. Daytime procession → 2.0 s
12. Glances of the crowd → 1.0 s

DAY TRANSITION → NIGHT (≈5 s)

13. Sunset → 2.0 s
14. Lights come on → 1.5 s
15. Illuminated statue → 1.5 s

FINALE – emotional climax (≈7 s)

16. Crowd in silhouette → 1.0 s
17. Reflections on the water → 1.0 s
18. Distant fires → 1.5 s
19. Boat with Saint Basso → 2.0 s
20. Night sea (closing) → 1.5 s

TOTAL: ~30 secondi
```

### AI Voice Generation

Background voice was generated with

[Gemini](https://gemini.google.com/)

using this prompt

```md
TODO PROMPT
```

### Background Music Generation

Background music was generated with

[SunoAI](https://suno.com/)

using this prompt

```md
TODO PROMPT
```

### Scene Prompts

#### Scene 01

- Title:
  - `Mare all’alba (inizio assoluto)`
- Framing:
  - `Campo lunghissimo, mare calmo`
- Initial Frame:
  - `Wide cinematic shot of calm sea at dawn in Termoli, Italy. Soft blue and golden tones, morning haze, no people. Minimal, quiet, contemplative. Film look, realistic, 16:9.`
- Final Frame:
  - `Same sea at dawn, light slightly brighter, harbor faintly visible. Still calm, timeless atmosphere.`
- Video Prompt:
  - `Slow forward camera movement over calm sea. Light gradually increases, very subtle motion.`

#### Scene 02

- Title:
  - `Porto che si sveglia`
- Framing:
  - `Campo lungo sul porto`
- Initial Frame:
  - `Early morning harbor of Termoli, empty docks, soft light.`
- Final Frame:
  - `Same harbor, first lights on, faint human presence.`
- Video Prompt:
  - `Very slow pan, atmosphere waking up.`

#### Scene 03

- Title:
  - `Dettaglio acqua sul molo`
- Framing:
  - `Close-up poetico`
- Initial Frame:
  - `Close-up of seawater touching stone dock, morning light.`
- Final Frame:
  - `Ripples slightly stronger, light reflections.`
- Video Prompt:
  - `Subtle water movement, macro cinematic realism.`

#### Scene 04

- Title:
  - `Volti anonimi (attesa)`
- Framing:
  - `Mezzo busto, spalle`
- Initial Frame:
  - `People seen from behind, looking at the sea, soft daylight. No faces visible.`
- Final Frame:
  - `Same people, light warmer, slight movement.`
- Video Prompt:
  - `Gentle handheld feel, slow breathing motion.`

#### Scene 05

- Title:
  - `Campane in lontananza`
- Framing:
  - `Dettaglio simbolico`
- Initial Frame:
  - `Church bell tower in Termoli, early morning light.`
- Final Frame:
  - `Bell tower slightly brighter, birds flying.`
- Video Prompt:
  - `Slow upward tilt, calm pace.`

#### Scene 06

- Title:
  - `Preparativi al porto`
- Framing:
  - `/`
- Initial Frame:
  - `Fishermen preparing boats, calm gestures, morning light.`
- Final Frame:
  - `More activity, ropes moving, people interacting.`
- Video Prompt:
  - `Slow cinematic montage-style motion.`

#### Scene 07

- Title:
  - `Bandierine e decorazioni`
- Framing:
  - `/`
- Initial Frame:
  - `Close-up of decorations moving in the wind near the harbor.`
- Final Frame:
  - `Decorations more numerous, sunlight stronger.`
- Video Prompt:
  - `Wind-driven motion, shallow depth of field.`

#### Scene 08

- Title:
  - `Statua di San Basso (prima apparizione)`
- Framing:
  - `/`
- Initial Frame:
  - `Statue of Saint Basso partially visible, soft daylight.`
- Final Frame:
  - `Statue fully framed, reverent atmosphere.`
- Video Prompt:
  - `Slow reveal, respectful pacing.`

#### Scene 09

- Title:
  - `Mani che toccano l’acqua`
- Framing:
  - `/`
- Initial Frame:
  - `Close-up of a hand touching seawater.`
- Final Frame:
  - `Water ripples expand, light reflections.`
- Video Prompt:
  - `Very slow motion, emotional realism.`

#### Scene 10

- Title:
  - `Barca che parte`
- Framing:
  - `/`
- Initial Frame:
  - `Fishing boat still at the dock, daylight.`
- Final Frame:
  - `Boat slowly moving away from the harbor.`
- Video Prompt:
  - `Smooth dolly movement following the boat.`

#### Scene 11

- Title:
  - `Processione in mare – giorno`
- Framing:
  - `/`
- Initial Frame:
  - `Boats aligned on calm sea, daytime.`
- Final Frame:
  - `Boats moving together, symbolic formation.`
- Video Prompt:
  - `Aerial-like slow movement (no drone feel).`

#### Scene 12

- Title:
  - `Sguardi tra la folla`
- Framing:
  - `/`
- Initial Frame:
  - `People watching the sea, faces partially hidden.`
- Final Frame:
  - `More people, shared emotion.`
- Video Prompt:
  - `Slow lateral movement, human warmth.`

#### Scene 13

- Title:
  - `Transizione giorno → sera`
- Framing:
  - `/`
- Initial Frame:
  - `Sea at late afternoon, warm tones.`
- Final Frame:
  - `Same view at sunset, orange sky.`
- Video Prompt:
  - `Time passing naturally, no jump cuts.`

#### Scene 14

- Title:
  - `Luci che si accendono`
- Framing:
  - `/`
- Initial Frame:
  - `Harbor at dusk, first lights on.`
- Final Frame:
  - `Night lights reflecting on water.`
- Video Prompt:
  - `Gradual illumination, cinematic contrast.`

#### Scene 15

- Title:
  - `Statua illuminata – notte`
- Framing:
  - `/`
- Initial Frame:
  - `Saint Basso statue at night, soft lights.`
- Final Frame:
  - `Lights brighter, dramatic shadows.`
- Video Prompt:
  - `Slow push-in, reverent tone.`

#### Scene 16

- Title:
  - `Silhouette della folla`
- Framing:
  - `/`
- Initial Frame:
  - `Crowd silhouettes at the harbor, night.`
- Final Frame:
  - `More people filling the frame.`
- Video Prompt:
  - `Gentle camera sway, emotional weight.`

#### Scene 17

- Title:
  - `Riflessi sull’acqua`
- Framing:
  - `/`
- Initial Frame:
  - `Reflections of lights on dark sea.`
- Final Frame:
  - `Reflections distorted by movement.`
- Video Prompt:
  - `Hypnotic, slow motion water dynamics.`

#### Scene 18

- Title:
  - `Fuochi lontani`
- Framing:
  - `/`
- Initial Frame:
  - `First fireworks glow in the sky.`
- Final Frame:
  - `Fireworks fully visible, reflected on water.`
- Video Prompt:
  - `Elegant, not chaotic fireworks motion.`

#### Scene 19

- Title:
  - `Barca con San Basso (climax)`
- Framing:
  - `/`
- Initial Frame:
  - `Boat carrying Saint Basso, centered, night.`
- Final Frame:
  - `Boat moving forward, powerful framing.`
- Video Prompt:
  - `Slow heroic forward movement.`

#### Scene 20

- Title:
  - `Chiusura – mare di notte`
- Framing:
  - `/`
- Initial Frame:
  - `Calm sea at night, lights fading.`
- Final Frame:
  - `Almost dark sea, quiet, still.`
- Video Prompt:
  - `Fade-like motion, emotional closure.`
